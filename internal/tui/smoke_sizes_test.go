...
func seedEverything(m *Model) {
	*m = func() Model {
		...
		mm.logs = mm.logs.SetSystemLogs([]models.SystemLogEntry{{Time: time.Now(), Severity: "high", Description: "d"}}, nil)
		mm.logs = mm.logs.SetTrafficLogs([]models.TrafficLogEntry{{Time: time.Now(), Action: "allow", SourceIP: "1.1.1.1"}}, nil)
		mm.logs = mm.logs.SetThreatLogs([]models.ThreatLogEntry{{Time: time.Now(), Severity: "high", ThreatName: "t"}}, nil)
		return mm
	}()
}

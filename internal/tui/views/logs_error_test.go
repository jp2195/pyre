...
func TestLogsModel_ErrorIsScopedToItsLogType(t *testing.T) {
	...
	m, _ = m.SetTrafficLogs(nil, LogPageMeta{}, errors.New("traffic-fetch-failed"))
...
func TestLogsModel_PerTypeErrorsDoNotOverwrite(t *testing.T) {
	...
	m := NewLogsModel()
	m, _ = m.SetSystemLogs(nil, LogPageMeta{}, sysErr)
	m, _ = m.SetThreatLogs(nil, LogPageMeta{}, threatErr)
	m, _ = m.SetTrafficLogs([]models.TrafficLogEntry{{Time: time.Now()}}, LogPageMeta{}, nil)

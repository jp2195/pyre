...
func NewIPSecTunnelsModel() IPSecTunnelsModel {
	config := RuleListConfig[models.IPSecTunnel]{
		...
		FormatRow:         formatIPSecRow,
		RenderDetail:      renderIPSecDetail,
		IsDisabled:        func(models.IPSecTunnel) bool { return false },
...
func formatIPSecRow(t models.IPSecTunnel, width int) string {
...
// styleTunnelRow colors non-selected rows by tunnel state using cached
// package styles (no per-row style construction in the render path).
func styleTunnelRow(t models.IPSecTunnel, row string) string {
	switch t.State {
	case "up":
		return TunnelUpRowStyle.Render(row)
	case "init":
		return TunnelInitRowStyle.Render(row)
	case "down":
		return TunnelDownRowStyle.Render(row)
	default:

...
func (m InterfacesModel) formatInterfaceRow(iface models.Interface, width int) string {
	...
	if width >= 120 {
		return fmt.Sprintf("%-2s %-16s %-10s %-12s %-18s %-17s %-12s",
			status, truncateEllipsis(name, 16), truncateEllipsis(ifType, 10),
			truncateEllipsis(zone, 12), truncateEllipsis(ip, 18), truncateEllipsis(mac, 17),
			truncateEllipsis(vr, 12))
	} else if width >= 90 {
		return fmt.Sprintf("%-2s %-14s %-8s %-10s %-16s %-12s",
			status, truncateEllipsis(name, 14), truncateEllipsis(ifType, 8),
			truncateEllipsis(zone, 10), truncateEllipsis(ip, 16), truncateEllipsis(vr, 12))
	}
	return fmt.Sprintf("%-2s %-14s %-10s %-16s",
		status, truncateEllipsis(name, 14), truncateEllipsis(zone, 10), truncateEllipsis(ip, 16))
}
